...
class FacturationController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $tdv = strip_tags($request->input('TDV'));
        ...
        if ($specialty_tranches->isEmpty()) {
            ...
            if ($sumRemainingAmounts == 0) {
                ...
                // dump($newFacturation);
                $newFacturation->tdv = $tdv;
                $newFacturation->academic_year = $year_session;
                $newFacturation->level_id = $level_id;
                ...
                $paymentSummary = facturation::where('student_id', $student_id)->where('level_id', $level_id)->where('academic_year', $year_session)->orderBy('id', 'asc')->get();
                $pdf = Pdf::loadView('facture.pdf', compact('student', 'specialty', 'level', 'year_session', 'period', 'paymentSummary','tdv', 'tranches', 'specialty_tranches', 'deposited_amount', 'deposit_date', 'paidAmounts', 'remainingAmounts', 'referenceId'));
    ...
    public function updateFacture(Request $request)
    {
        ...
        $montant = strip_tags($request->input('upMontant'));
        $student_id = facturation::find($id)->student_id;
        $student = student::findOrFail($student_id);
        $year_session = Session::get('year_name');
        ...
        if ($specialty_tranches->isEmpty()) {
            ...
            if ($sumRemainingAmounts == 0) {
                ...
                $pdf = Pdf::loadView('facture.pdf', compact('student', 'specialty', 'level', 'year_session', 'period', 'paymentSummary', 'tranches', 'specialty_tranches', 'deposited_amount', 'deposit_date', 'paidAmounts', 'remainingAmounts', 'referenceId'));
                $pdf->setPaper('a4', 'landscape');
